...
\title{ReSkip: Drop-in Adaptive Depth for Pretrained Transformers\\via Attention Residuals}
...
\begin{abstract}
Different inputs deserve different amounts of computation---a System-1 / System-2 intuition with deep cognitive and neural roots~\citep{kahneman2011thinking, lamme2000distinct, kar2019recurrent}.
Recent test-time compute scaling for LLMs operationalises this intuition along the \emph{token} axis (more reasoning tokens for harder problems)~\citep{openai2024o1, deepseek2025r1, snell2024scaling, wei2022chain}, but every token in a long reasoning trace still goes through every layer of the transformer.
The complementary \emph{depth} axis---which blocks to invoke per input---is largely under-exploited, and existing methods (early exit, mixture-of-depths, layer pruning) bolt on a separate decider rather than producing the depth-allocation signal as a property of the forward pass.
We use \emph{Attention Residuals} (\attnres{})~\citep{chen2026attnres}---a residual that attends over previous block outputs and so emits a per-block routing weight as a side-effect of forming each block's input---as the intrinsic handle, and develop a recipe to install it into any pretrained transformer.
\textbf{(1)~\reskip{}.} We read the \attnres{} weight as a pre-execution skip signal: input-dependent, position-calibrated, no auxiliary head ($1.19\times$ wall-clock at $340$M from-scratch, zero benchmark drop).
\textbf{(2)~\retrofit{} (main contribution).} A $\gamma$-gated residual-injection fine-tune converts \emph{any} pretrained decoder transformer into an \attnres{}-capable model: identity-at-init, base frozen, $\sim$$0.7\%$ new parameters, $\sim$$22$ GPU-minutes on $2$B. On Qwen3-VL-2B/4B the retrofit strictly improves base on $5$/$6$ full \texttt{lmms-eval} benchmarks at both scales ($+3.1$pp MMBench, $+1.8$pp MMMU on $2$B; $+1.9$pp MMBench, $+3.1$pp MMMU on $4$B), with the largest single-cell gain on the MMStar math subcategory ($+7.9$pp on $2$B).
\textbf{(3)~VLA.} A LIBERO policy warm-started from the retrofit beats a matched pure-OFT baseline at both $2$B and $4$B; \reskip{} on top of the warm-start \emph{beats} the no-skip backbone on the LIBERO 4-suite mean ($+1.10$pp on $2$B, $+0.20$pp on $4$B at the conservative operating point).
\textbf{(4)~Iso-cost.} All accuracy results above are obtained at the same fast forward path: under \texttt{torch.compile} max-autotune, the retrofit lands at $1.03\times$ base\_compiled and $0.864\times$ of the stock-HF base eager forward most deployments use today---there is no speed/accuracy trade-off to defend.
The recipe is, to our knowledge, the cheapest existing path from an off-the-shelf transformer to a depth-axis-System-2-capable model, and is complementary to token-axis test-time-compute approaches at two orders of magnitude less compute.
\end{abstract}
...
\begin{itemize}
\item \textbf{\reskip{}} (\S\ref{sec:reskip}). The phase-1 \attnres{} weights drive a per-input skip rule with position-calibrated thresholds, no auxiliary head. On a 340M from-scratch \attnres{} transformer this yields $1.19\times$ wall-clock at zero benchmark drop. We document an \emph{importance--ablation disconnect}: combining the routing-importance and static-removal signals to pick eligible positions strictly dominates either alone.
\item \textbf{\retrofit{}} (\S\ref{sec:retrofit}, main contribution). A $\gamma$-gated residual injection installs \attnres{} into a frozen pretrained transformer through a single short fine-tune: identity-at-init, $\sim$$0.7\%$ new parameters, $\sim$$22$ GPU-minutes on Qwen3-VL-2B. Every block converges to $\gamma_n{=}1$ (\emph{structurally pure} \attnres{}). On Qwen3-VL-2B/4B the retrofit strictly improves base on $5$/$6$ full \texttt{lmms-eval} benchmarks at both scales (e.g., $+3.1$pp MMBench, $+1.8$pp MMMU on 2B; $+1.9$pp MMBench, $+3.1$pp MMMU on 4B), with the MMStar math subcategory swinging $+7.9$pp on $2$B---the largest single cell across the suite.
\item \textbf{VLA application} (\S\ref{sec:vla}). A LIBERO policy warm-started from the retrofit beats a matched pure-OFT baseline at both 2B and 4B; a control that skips the retrofit and runs the $\gamma$-curriculum directly on VLA data plateaus $1.65$pp behind even at $2\times$ the VLA compute. \emph{Crucially, \reskip{} on top of the warm-start beats the no-skip backbone on the 4-suite mean} ($+1.10$pp on $2$B, $+0.20$pp on $4$B at the conservative operating point), reproducing on robotic action what we see for next-token prediction at $340$M.
\item \textbf{Iso-cost speed} (\S\ref{sec:exp_retrofit}, ``Inference cost''). The accuracy results above are obtained at the same fast inference path. Under \texttt{torch.compile} (max-autotune, seq~$2048$, KV cache enabled), the retrofit forward measures $1.029\times$ base\_compiled and $0.864\times$ of the stock-HF base eager forward most deployments use. The paper's headline is therefore ``$+$accuracy at iso-cost'' rather than a speed/accuracy trade-off.
\end{itemize}

\paragraph{Retrofit-as-tool: depth-axis System 2 for any pretrained transformer.}
\retrofit{} converts an off-the-shelf pretrained transformer into a depth-axis-System-2-capable model through a single short fine-tune. The base weights are not modified; the same recipe applies to text-only, VL, and VLA backbones; the resulting model inherits \reskip{}'s skip rule with no further training. This is, to our knowledge, the cheapest existing path to depth-axis System~2 for an already-trained transformer, and is complementary to token-axis approaches: a model that has learnt to allocate compute through reasoning tokens (via RL or chain-of-thought training) can additionally acquire depth-axis allocation through our retrofit, with the latter costing two orders of magnitude less compute.
...
On a 340M block-\attnres{} transformer ($d{=}1024$, $L{=}24$, $N{=}8$ blocks; FineWeb-Edu 100BT~\citep{penedo2024fineweb}; \texttt{lm-eval-harness}~\citep{eval-harness}), \reskip{} at $\mathcal{P}{=}\{3,5\}, M_{\max}{=}2, q{=}0.85$ reaches \textbf{$1.19\times$ wall-clock at zero benchmark drop} on LAMBADA~\citep{paperno2016lambada}, HellaSwag~\citep{zellers2019hellaswag}, and ARC-E/C~\citep{clark2018arc} (Table~\ref{tab:reskip_benchmark}; Pareto and latency curves in Appendix~\ref{app:reskip_results}). Skip count varies $0$--$2$ per forward (mean $0.88$ over $48$ batches), confirming genuine input-dependence rather than a fixed-block removal. \emph{This validates that the mechanism is real when \attnres{} is trained in. The harder question---how to acquire it in an already-trained model---is \S\ref{sec:retrofit}.}
...
%----------------------------------------------------------------------
\section{\retrofit{}: Installing AttnRes into a Pretrained Transformer}
\label{sec:retrofit}
%----------------------------------------------------------------------
...
\end{equation}
where the skip-branch KL samples one block at random per step, runs the forward with it skipped, and pulls the resulting logits toward a frozen copy of the pretrained teacher (so the surrogate $x_n$ is useful both when the block runs and when it is skipped). $\mathcal{L}_{\text{ent}}$ is a light prior against degenerate $\alpha$. Default $\lambda_{\text{kl}}{=}1$, $\lambda_{\text{ent}}{=}0.02$, lr $10^{-3}$ on retrofit params, cosine schedule with $100$-step warmup. Hyperparameters and the v1 / v3 data mixes used throughout are listed in Appendix~\ref{app:retrofit_hparams}.
...
We organise results into two parts: \S\ref{sec:exp_retrofit} on the retrofit (the main contribution, on Qwen3-VL-2B/4B), and \S\ref{sec:vla} on the downstream VLA application. The from-scratch 340M validation of \reskip{} (\S\ref{sec:reskip}) plays the role of an existence proof for the routing mechanism; cross-scale (110M done, 1.3B/2B planned) results are in Appendix~\ref{app:phase1}.

\subsection{Phase 1: From-scratch \attnres{} (summary)}
\label{sec:exp_scale}

The 340M from-scratch \attnres{} validation of \S\ref{sec:reskip} (Tab.~\ref{tab:reskip_benchmark}) gives three properties Phase~2 needs to inherit: \emph{dynamic skip at $q{=}0.95$ preserves every metric}, \emph{the importance--ablation disconnect} requires both signals to pick $\mathcal{P}$, and \emph{no auxiliary classifier is needed} because phase-1 statistics are pre-execution. We also have a 110M cross-scale replication on the same FineWeb-Edu recipe at $q{=}0.97, M_{\max}{=}1$ (Appendix~\ref{app:phase1}); 1.3B/2B from-scratch are planned and not load-bearing for the main contribution, which targets a pretrained $2.13$B model directly.
\subsection{\retrofit{} on Qwen3-VL-2B and 4B}
\label{sec:exp_retrofit}

\paragraph{Setup.}
We retrofit \textbf{Qwen3-VL-2B}~\citep{bai2025qwen3vl} ($L{=}28, d{=}2048$) at $N{=}7$ blocks of $L{=}4$ and \textbf{Qwen3-VL-4B} ($L{=}36, d{=}2560$) at $N{=}9$ blocks of $L{=}4$ (block-partition sweep in Appendix~\ref{app:block_partition}). $L{=}4$ is the canonical partition for both scales: it sits on the accuracy plateau alongside $L{=}2$ and halves the per-token router count, which is the dominant factor in the inference-cost result below. All base parameters are frozen; $\sim$$15$M ($2$B) / $\sim$$23.7$M ($4$B) retrofit parameters at $r{=}256$ train via Eq.~\ref{eq:retrofit_loss}. We compare two data mixes: \textbf{v1} ($50$/$50$ UltraChat-$200$k~\citep{ding2023ultrachat} + LLaVA-Instruct-VSFT~\citep{liu2023llava}, $5$k steps; $\sim$$22$ GPU-min on $2$B), used as a LoRA-comparable narrow-mix control; and \textbf{v3} ($60\%$ LLaVA-OneVision-Data~\citep{li2024llavaonevision} + $20\%$ UltraChat + $10\%$ NuminaMath + $10\%$ OpenThoughts, $10$k steps), the canonical mix used for every result we report. Evaluation: LAMBADA~\citep{paperno2016lambada}/HellaSwag~\citep{zellers2019hellaswag} for text; six \texttt{lmms-eval} benchmarks for VLM (MMBench~\citep{liu2023mmbench}, MMMU~\citep{yue2024mmmu}, MMStar~\citep{chen2024mmstar}, AI2D~\citep{kembhavi2016ai2d}, OCRBench~\citep{liu2024ocrbench}, RealWorldQA~\citep{grok2024realworldqa}); \reskip{} rule (Eq.~\ref{eq:skip_rule}) eligible-block sets selected per-scale from per-block static-removal (Appendix~\ref{app:block_removal}). Identity-at-init verified ($\gamma{=}0$ reproduces base; max $|\Delta\text{logits}|=0.375$ bf16, $100\%$ argmax agreement; Appendix~\ref{app:identity}).
...
\paragraph{Findings.}
\textbf{(V1) The warm-start is net positive at both scales.} Path B reaches $97.15\%$ on $2$B (Path B max / Path 0 min, $+1.30$pp; preserved at $+0.70$pp under same-seed averaging) and $96.70\%$ on the clean-base $4$B ($+0.65$pp). The gain concentrates on Spatial ($+3.0$pp $2$B) and Long-$10$ ($+2.0$pp $4$B).
\textbf{(V2) The retrofit is not substitutable by longer VLA training.} Path C plateaus at $94.30\%$ at $30$k, $2.85$pp behind Path B; at $60$k it closes only some of the gap ($95.50\%$, still $1.65$pp behind Path B $30$k). A $35$k-equivalent total ($5$k VLM retrofit + $30$k VLA) beats $60$k of VLA-only, on the suites where a stable pretrained router most matters---Spatial task $6$ goes from $50\%$ (Path C $30$k) and $64\%$ (Path C $60$k) to $78\%$ (Path B $30$k).
...
\begin{table}[t]
\caption{\textbf{\reskip{} 4-suite Pareto} on the trained Path B policies. Eligible blocks $\mathcal{P}$ and threshold $\tau$ calibrated per scale on the action distribution; $q$ is the empirical quantile that produces $\tau$. \emph{No-skip} reproduces the Path B row of Tab.~\ref{tab:vla_libero}. The conservative end (here $q{=}0.99$) is the recommended operating point: $+1.10$pp $2$B / $+0.20$pp $4$B over the no-skip backbone, while still saving $5$--$9\%$ wall-clock per call. The $4$B is graceful all the way to $q{=}0.30$ (only $-4.4$pp); $2$B collapses below $q{=}0.85$, consistent with the larger model carrying more depth-redundancy.}
...
\paragraph{Test-time compute scaling (token axis).}
o1~\citep{openai2024o1}, DeepSeek-R1~\citep{deepseek2025r1}, and the broader Snell-style optimal-compute analysis~\citep{snell2024scaling} show that allowing a model to spend more inference compute on harder problems improves accuracy substantially. Chain-of-thought~\citep{wei2022chain} and self-thought methods~\citep{zelikman2024quietstar} are the most common operationalisations. All of these allocate compute by emitting more reasoning \emph{tokens}, leaving per-token depth fixed. Our work is on the orthogonal \emph{depth} axis and is composable with token-axis methods: a model that already does long-chain reasoning can additionally acquire depth-axis allocation through our retrofit.

\paragraph{Adaptive computation in transformers (depth axis).}
ACT~\citep{graves2016adaptive} and Universal Transformers~\citep{dehghani2018universal} introduce learned halting; CALM~\citep{schuster2022confident} adds per-layer exit classifiers; Mixture-of-Depths~\citep{raposo2024mixture} learns a token router with a capacity loss; LayerSkip~\citep{elhoushi2024layerskip} combines early exit with self-speculative decoding. Each augments the network with a separate decider. Our work differs by using \attnres{}---a structural change to the residual---as the intrinsic routing signal, with no auxiliary component, and (the main contribution) by installing that signal into an existing pretrained transformer through a short fine-tune.

\paragraph{Cognitive and neural roots.}
The dual-process distinction between automatic System~1 and effortful System~2~\citep{kahneman2011thinking} is mirrored in cortical processing: easy stimuli are handled by shallow feedforward sweeps; hard ones recruit recurrent and deeper circuits~\citep{lamme2000distinct, kar2019recurrent, dicarlo2012does}; and the gating decision is intrinsic to canonical microcircuits~\citep{bastos2012canonical, buschman2007topdown}, not made by an external supervisor. We use this convergence as a design target rather than as a claim about cognitive faithfulness.

\paragraph{Layer pruning and skipping.}
\citet{gromov2024unreasonable} and ShortGPT~\citep{men2024shortgpt} show that many transformer layers can be removed post-training with modest loss. These approaches do static removal (no input dependence); our retrofit preserves the full network and routes per input.

\paragraph{Residual-connection innovations.}
DenseNet~\citep{huang2017densely} concatenates layer outputs; Highway~\citep{srivastava2015highway} uses gated residuals; \attnres{}~\citep{chen2026attnres} generalises these with attention-based weighting, which we adopt as the architectural building block. We are the first to expose \attnres{} weights as a routing signal and the first to retrofit the mechanism into pretrained models.

\paragraph{Vision-Language-Action models.}
RT-2~\citep{brohan2023rt}, Octo~\citep{team2024octo}, OpenVLA~\citep{kim2024openvla}, and Pi0~\citep{black2024pi0} established the VLA paradigm. Efficient-VLA work so far has focused on action chunking~\citep{zhao2023learning} and post-hoc quantisation; we contribute a modality-aware adaptive-depth mechanism for pretrained VLM backbones.
...
\paragraph{Summary.}
\reskip{} reads phase-1 \attnres{} weights as an intrinsic skip signal: $1.19\times$ wall-clock at $340$M from-scratch with zero benchmark drop, plus an importance--ablation disconnect that requires combining both signals to pick eligible positions. \retrofit{} (the main contribution) installs \attnres{} into a frozen pretrained transformer through a single short fine-tune ($\sim$$0.7\%$ new parameters, identity-at-init, every block converges to $\gamma_n{=}1$): on Qwen3-VL-2B/4B at the canonical $L{=}4$ v3 recipe it strictly improves $5$/$6$ \texttt{lmms-eval} VLM benchmarks at both scales (MMBench $+3.1$/$+1.9$pp, MMMU $+1.8$/$+3.1$pp), lifts LAMBADA by $+3.3$/$+8.7$pp, and concentrates the largest single-cell gain on MMStar math ($+7.9$pp on $2$B). A LIBERO VLA warm-started from the retrofit beats matched pure-OFT at both scales, and \emph{\reskip{} on the trained VLA policy beats the no-skip backbone on the 4-suite mean} ($+1.10$pp $2$B / $+0.20$pp $4$B at the conservative operating point). All accuracy results above are obtained at iso-cost: under \texttt{torch.compile} max-autotune, the retrofit forward measures $1.029\times$ \texttt{base\_compiled} and $0.864\times$ of the stock-HF \texttt{base\_eager} forward---the paper's framing is therefore \emph{$+$accuracy at iso-cost} rather than a speed/accuracy trade-off.
...
For Qwen3-VL-2B ($28$ decoder layers) we default to $N{=}14$ blocks of $L{=}2$ layers; for Qwen3-VL-4B ($36$ layers) we recommend $N{=}9$, $L{=}4$. The block-partition sweep of Appendix~\ref{app:block_partition} motivates both choices. Each block receives one router, one residual adapter ($r{=}256$ default), and a scalar $\gamma$ gate.
...
\subsection{Wall-clock latency: full table including legacy $L{=}2$ and VLA in-backbone}
\label{app:latency}
...
\begin{table}[h]
\caption{Block partition sweep (v3 recipe, $r{=}256$, $\gamma$-curriculum). $L$ is layers-per-block, $N{=}L_{\text{total}}/L$. Per-layer $L{=}1$ underperforms by $2$--$9$pp; the plateau $L\in\{2,4,7\}$ on $2$B reproduces \citet{chen2026attnres}'s $S\in\{2,4,8\}$. We recommend $L{=}2$ on $2$B and $L{=}4$ on $4$B.}
...
\textbf{2B (rec.)} & $\mathbf{2}$ & $\mathbf{14}$ & $\mathbf{0.5755}$ & $\mathbf{+4.4}$ & $\mathbf{0.494}$ & \textbf{77.23} & 0.426 & 0.532 & 0.748 & 0.803 & 0.663 \\
...
$+$ \retrofit{} v3 ($2$B, $L{=}2$)       & \textbf{0.492} & \textbf{0.432} & 0.353 & 0.734 & 0.505 & 0.683 \\
...
\paragraph{LAMBADA-$500$ accuracy parity.} Running LAMBADA-$500$ with the retrofit wrapped in \texttt{torch.compile(mode="default", dynamic=True)}: acc $0.5720$ compiled vs.\ $0.5700$ eager ($\Delta{=}+0.20$pp), ppl $4.534$ vs.\ $4.526$. Per-target argmax agreement against eager: $98.60\%$ — the residual $1.4\%$ are tokens where eager and compiled both fall within bf16 SDPA jitter and the rank-$1$/rank-$2$ logits flip.
