...
def eval_libero(args: Args) -> None:
    ...
    total_effective_block_ratio = 0.0
    total_routed_steps = 0
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        task_effective_block_ratio = 0.0
        task_routed_steps = 0
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            while t < max_steps + args.num_steps_wait:
                ...
                if "attnres_effective_block_ratio" in response:
                    ratio = float(response["attnres_effective_block_ratio"])
                    task_effective_block_ratio += ratio
                    total_effective_block_ratio += ratio
                    task_routed_steps += 1
                    total_routed_steps += 1
                    if step == 0 and "attnres_backbone_compute_preserved" in response:
                        logging.info(
        ...
        if task_routed_steps > 0:
            logging.info(
                f"Current task mean effective block ratio: {task_effective_block_ratio / float(task_routed_steps):.4f}"
            )
        logging.info(
    ...
    if total_routed_steps > 0:
        logging.info(
            f"Mean effective block ratio across all rollout steps: {total_effective_block_ratio / float(total_routed_steps):.4f}"
        )
